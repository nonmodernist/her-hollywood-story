...
import { renderDetailView } from './modules/detail-renderers.js';
import { createListItem, createGridItem } from './modules/list-renderers.js';
...
function setupEventListeners() {
    ...
    // Grid item navigation (event delegation)
    elements.resultsContainer.addEventListener('click', (e) => {
        const filmItem = e.target.closest('.film-item[data-film-slug]');
        if (filmItem) {
            const slug = filmItem.dataset.filmSlug;
            router.navigate(getDatabaseURL('/film/' + slug));
        }
    });
